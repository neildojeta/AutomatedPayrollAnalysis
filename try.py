...
def calculate_client_totals1(hours_sheet, pr_sheet, client):
    try:
        # Find the row index where the client name appears in the first column
        client_rows = pr_sheet[pr_sheet.iloc[:, 0].astype(str).str.contains(client, na=False, case=False)]
        logger.info(f"Calculating client totals for {client}.")
        # print("Client rows: ", client_rows)
        ...
        # Get all unique partners for this client
        total_partners = hours_sheet["PARTNER"].unique()
        # total_partners = hours_sheet[hours_sheet["CLIENT"] == client]["PARTNER"].unique()
        # total_partners = hours_sheet["PARTNER"].drop_duplicates()
        print("\ntotal partners: ", len(total_partners))
        total_amount = 0

        for partner in total_partners:
            # Find rows in PR sheet matching the partner
            # partner_rows = pr_sheet[pr_sheet.iloc[:, 0].astype(str).str.contains(str(partner), na=False, case=False)]
            partner_rows = pr_sheet[pr_sheet.iloc[:, 0].astype(str).str.strip() == str(partner).strip()]

            if not partner_rows.empty:
                # Find the total row index (assuming the amount is in column 14)
                print("\nAmount: ", partner_rows.iloc[0, 14])
                total_amount += partner_rows.iloc[0, 14]  # Column 14 contains the amount
